...
from seeder.pokeapi_client import DEFAULT_MULTIPLIER, PokeAPIClient, PokemonData, TypeData
...
def run(limit: int = 151, drop: bool = False, database_url: str | None = None) -> None:
    ...
    logger.info("-" * 50)
    logger.info("Listo: %s tipos, %s relaciones, %s pokemon", len(type_ids), relations, saved)
...
def main() -> int:
    ...
    try:
        run(limit=args.limit, drop=args.drop, database_url=args.database_url)
